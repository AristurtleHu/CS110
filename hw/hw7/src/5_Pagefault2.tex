...
\begin{parts}
    ...
    \begin{itemize}
        ...
        \item The number of page faults triggered is \blank[3cm]{3}.
              \vspace{0.5cm}
        \item Fault addresses (page base address): \blank[7cm]{0x392000, 0x39300, 0x394000}.
    \end{itemize}

    \part[8] If the page size is 128 bytes, and the array is stored in column-major order, after execution: [8 points]
    \begin{itemize}
        \vspace{0.5cm}
        \item The number of page faults triggered is \blank[3cm]{49}.
              \vspace{0.5cm}
    \end{itemize}
    \vspace{0.5cm}
    ...
    The page table entry (PTE) is 32 bits. The maximum number of PPN is $2^{32-8} = 2^{24}$, and each PTE can map to a physical frame of size $2^{14}$ bytes. Therefore, the maximum physical memory size is: $$2^{24} \times 2^{14} = 2^{38} \text{ bytes} = 256 \text{ GB}.$$
