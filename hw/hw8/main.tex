...
\runningheader{Email: yangry2023}
...
\begin{questions}
    ...
    \begin{parts}
        ...
        \begin{figure}[h]
            ...
            \hfill
            \begin{minipage}[t]{0.35\textwidth}
                \includegraphics[width=\linewidth]{assets/number_two.png}
...
\end{lstlisting}}
        ...
        \underline{slli t0, x14, 2}\\
        \underline{add t1, t0, x12}\\
        \underline{lw t2, 0(t1)}\\
        \underline{lb t3, 0(t2)}\\
        \underline{lbu t4, 1(t2)}\\
        \underline{lh t5, 2(t2)}\\
        \underline{mul t4, t3, t4}\\
        \underline{add t6, t5, t4}\\
        \underline{sw t6, 4(t2)}\\
...
\end{lstlisting}}
    ...
    \begin{parts}
        ...
        \begin{align*}
            t_{IF}  & = t_{clk-to-q} + t_{setup}+ t_{I-memory-read} = 250ps                 \\
            t_{ID}  & = t_{clk-to-q} + t_{setup} + max(t_{regfile-read}, t_{imm}) = 150ps   \\
            t_{EX}  & = t_{clk-to-q} + t_{setup} + t_{mux} + t_{alu} = 225ps                \\
            t_{MEM} & = t_{clk-to-q} + t_{setup} + max(t_{D-memory-read}, t_{mux})  = 250ps \\
            t_{WB}  & = t_{clk-to-q} + t_{mux} + t_{regfile-setup} = 70ps
        \end{align*}
        The longest delay of the five stages is Memory read:
        \[
        ...
        \begin{align*}
            t_{clk} & \ge t_{clk-to-q}+ t_{I-memory-read} + t_{regfile-read} + t_{mux} + t_{alu} + t_{D-memory-read} + t_{mux} + t_{regfile-setup} \\
                    & = 25ps + 200ps + 100ps + 25ps + 150ps + 200ps + 25ps + 20ps =  745ps
        \end{align*}
...
\end{lstlisting}}
        ...
        \newline
        \underline{$I1\rightarrow I3$}\\
        \underline{$I3\rightarrow I5$}\\
        \underline{$I5\rightarrow I6$}\\
        \underline{$I7\rightarrow I8$}\\
    ...
    \begin{parts}
        ...
        \begin{align*}
            \text{L2 local miss rate} & = \frac{\text{L2 misses}}{\text{L2 accesses}} = \frac{5}{100 - 80} = \frac{5}{20} = 0.25
        \end{align*}
